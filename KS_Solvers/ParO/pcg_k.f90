...
SUBROUTINE pcg_k( hs_1psi_ptr, g_1psi_ptr, psi0, spsi0, npw, npwx, nbnd, npol, psi, ethr, iter, e, nhpsi )
  ...
  INTEGER,  INTENT(IN)   :: npw, npwx, nbnd, npol, iter ! input dimensions and iteration count
  REAL(DP), INTENT(IN)   :: ethr                  ! threshold for convergence.
  REAL(DP), INTENT(IN)   :: e                     ! current estimate of the target eigenvalue
  COMPLEX(DP),INTENT(INOUT) :: psi(npwx*npol)     ! input: the current estimate of the eigenvector,
  ...
  COMPLEX(DP), ALLOCATABLE ::  spsi0vec (:) ! the product of spsi0 and a trial vector
  ...
  kdmx  = npwx* npol
  !
  ALLOCATE( hpsi( kdmx ),  spsi( kdmx ), r( kdmx ), z( kdmx ), b( kdmx ) )
...
!-
  ...
  ALLOCATE( p( kdmx ), sp( kdmx ), w( kdmx ) )
...
! remove an occupied orb contribution which is taken care of by the following rotate_wfc routine anyway
  ...
  !IF ( ABS ( g1 ) < ethr_cg) write (*,*) 'CG correction converged ', cg_iter, g1, g0, ethr_cg
  !IF ( ABS ( g1 ) > ethr_cg) write (*,*) 'CG not converged ', cg_iter, g1, g0, ethr_cg
