...
SUBROUTINE diago_cg_g(ndim,omat,smat,maxter,max_state,e,ovec,cutoff,ethr,found_state,l_para)
  !----------------------------------------------------------------------------
  !
  ! ... "poor man" iterative diagonalization of a genralized real symmetric  matrix O and  S
  ...
  REAL(kind=DP), INTENT(in) :: smat(ndim,ndim)!overlap matrix 
...
!initialization
  DO iw = 1, max_state
   ...
   DO m = 1, max_state
     ...
     !aux(:,2)=ovec(:,m)
     call gradient_s(ovec(1:ndim,m),aux(1:ndim,2))
...
states:  DO m = 1, max_state
  ...
  FLUSH(stdout)
     ...
     call gradient_s(ovec(1:ndim,m),spsi(1:ndim))
     !
     ...
     iterate: DO iter = 1, maxter
       ...
       ! scg(1:ndim)=g(1:ndim)
        call gradient_s(g(1:ndim),scg(1:ndim))
        !
        ...
        !scg(1:ndim)=cg(1:ndim)
         call gradient_s(cg(1:ndim),scg(1:ndim))
        !
...
!apply gradient
  ...
  !
   SUBROUTINE gradient_s(vec,grad)
!apply S matrix
      implicit none
      ...
      grad(:)=0.d0
      if(.not.l_para) then
         call dgemm('T','N',nsize,1,ndim,1.d0,smat(1:ndim,nbegin:nend),ndim,vec,ndim,0.d0,grad(nbegin:nend),nsize)
      else
         call dgemm('T','N',nsize,1,ndim,1.d0,smat(1:ndim,1:nsize),ndim,vec,ndim,0.d0,grad(nbegin:nend),nsize)
      endif
