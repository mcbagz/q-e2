...
!
!-----------------------------------------------------------------------
subroutine compute_alphasum_cond
  !-----------------------------------------------------------------------
  ...
  !! term and a part of the US contribution to the dynamical matrix, only for conduction bands. 
  !
  !! It implements Eq.(B17) of PRB 64, 235118 (2001). This quantity is distributed
  !! among processors.
  !
  !
  ...
  USE wvfct,      ONLY : nbnd, wg, nbnd_cond
  ...
  alphasum_cond  = 0.d0
  IF (noncolin) alphasum_cond_nc=(0.d0,0.d0)
  do ik = 1, nksq
     ikk = ikks(ik)
     ikq = ikqs(ik)
           ...
           do na = 1, nat
              if (ityp (na) == nt) then
                 ...
                 do ih = 1, nh (nt)
                    ikb = ijkb0 + ih
                    ijh = ijh + 1
                    do ibnd = nbnd-nbnd_cond+1, nbnd_occ (ikk)
                       wgg1 = wg (ibnd, ikk)
                       do ipol = 1, 3
                          IF (noncolin) THEN
                             DO is1=1,npol
                                DO is2=1,npol
                                   alphasum_cond_nc(ijh,ipol,na,is1,is2) =         &
                                      alphasum_cond_nc(ijh,ipol,na,is1,is2)+wgg1*  &
                             ...
                             END DO
                          ELSE
                             alphasum_cond(ijh,ipol,na,current_spin) = &
                               alphasum_cond(ijh,ipol,na,current_spin) + 2.d0*wgg1*&
           ...
           DO na = 1, nat
              IF (ityp(na)==nt) THEN
                 IF (upf(nt)%has_so) THEN
                    CALL transform_alphasum_so(alphasum_cond_nc,na)
                 ELSE
                    CALL transform_alphasum_nc(alphasum_cond_nc,na)
                 END IF
              END IF
           END DO
        END IF
