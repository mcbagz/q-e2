...
subroutine addusddens_cond (drhop, dbecsum, mode0, npe, iflag)
  ...
  USE lr_two_chem, ONLY : becsum_cond,alphasum_cond,becsumort_cond
  implicit none
  !
  !   the dummy variables
  !
  ...
  if (.not.okvan) return
  call start_clock ('addusddens_cond')
                    ...
                    do ipert = 1, npe
                       do is = 1, nspin_mag
                          ...
                          if (abs(u1) + abs(u2) + abs(u3) .gt.1d-12 .and. &
                              iflag.eq.1) then
                             bb = becsum_cond (ijh, na, is)
                             zsum = zsum + &
                                  ( alphasum_cond (ijh, 1, na, is) * u1 &
                                  + alphasum_cond (ijh, 2, na, is) * u2 &
                                  + alphasum_cond (ijh, 3, na, is) * u3)
                             IF (okpaw) becsumort_cond(ijh,na,is,mode) =  zsum
                             u1 = u1 * fact
                             u2 = u2 * fact
                             u3 = u3 * fact
                             ...
                             IF (okpaw.and.iflag==1) &
                                    becsumort_cond(ijh,na,is,mode) = zsum
                          endif
                       enddo
                    enddo
                 endif
  ...
  call stop_clock ('addusddens_cond')
  return
