...
!
! This is the routine epsilon.f90. We make a small modification so that 
                ...
                DO iband1 = nbndmin,nbndmax
                    ...
                    IF ( focc(iband1,ik) >= 0.5d-4*full_occ ) THEN
                        ...
                        DO iw = 1, nw
                            ...
                            w = wgrid(iw)
                            !
                            ! mz_modifies: to have Gaus broad for epsi2 
                            epsi(:,iw) = epsi(:,iw) + dipole(:,iband1,iband2) / etrans**2   & 
                                                  * RYTOEV**3 * (focc(iband1,ik))   &
                                        / intersmear / sqrt(3.141592653589793) * (EXP(-(etrans - w)**2 / intersmear**2))
