  ...
  CONTAINS
    ...
    !
    IF (epw_memdist) THEN
      ! Use memory-distributed epmatwp_dist
      CALL ephwan2blochp_dist(nmodes, xxq, irvec_g, nrr_g, epmatf, nbnd, nrr_k)
      RETURN
    ENDIF
    !
    CALL start_clock('ephW2Bp')
    !
    ALLOCATE(cfac(dims, nat, nrr_g), STAT = ierr)
    IF (ierr /= 0) CALL errore('ephwan2blochp', 'Error allocating cfac', 1)
    ALLOCATE(eptmp(nbnd, nbnd, nrr_k, nmodes), STAT = ierr)
    IF (ierr /= 0) CALL errore('ephwan2blochp', 'Error allocating eptmp', 1)
    !
    !----------------------------------------------------------
    ...
    IF (etf_mem == 0) then
      ...
      DO irn = ir_start, ir_stop
        ...
        cfac_new = EXP(ci * rdotk)
        CALL ZAXPY(nbnd * nbnd * nrr_k, cfac_new, epmatwp(:, :, :, imode, ir), 1, eptmp(:, :, :, imode), 1)
      ENDDO
    ...
    ENDIF ! etf_mem
    !
    CALL mp_sum(eptmp, inter_pool_comm)
    !
    epmatf = eptmp
    !
    ...
    IF (ierr /= 0) CALL errore('ephwan2blochp', 'Error deallocating eptmp', 1)
    !
    ...
    !---------------------------------------------------------------------------
    !!
    !! This subroutine optimizesmemory usage by distributing the memory 
    ...
    USE global_var,       ONLY : epmatwp_dist
    ...
    DO irn = ir_start, ir_stop
      ...
      rdotk = twopi * DOT_PRODUCT(xxq, DBLE(irvec_g(:, ir)))
      cfac_new = EXP(ci * rdotk)
      CALL ZAXPY(nbnd * nbnd * nrr_k, cfac_new, epmatwp_dist(:, :, :, irn - ir_start + 1), 1, epmatf(:, :, :, imode), 1)
    ENDDO
    !
