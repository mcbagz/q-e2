  ...
  CONTAINS
    ...
    USE input,         ONLY : nbndsub, fsthick, eps_acoustic, degaussw, restart,      &
                              nstemp, scattering_serta, scattering_0rta, shortrange, &
                              restart_step, restart_filq, assume_metal
    USE pwcom,         ONLY : ef
    USE global_var,    ONLY : ibndmin, etf, nkf, nkqf, vmef, wf, wqf, &
                              epf17, nkqtotf, inv_tau_all, inv_tau_allcb,    &
                              xqf, zi_allvb, zi_allcb, nbndfst, nktotf, gtemp, &
                              lower_bnd
    USE ep_constants,  ONLY : zero, one, two, ryd2mev, kelvin2eV, ryd2ev,        &
                              eps6, eps8, eps4
