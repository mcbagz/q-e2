  ...
  CONTAINS
    ...
    USE ep_constants,  ONLY : kelvin2eV, zero, two, ryd2ev, ryd2mev, ci
    USE ep_constants,  ONLY : pi
    USE io_global,     ONLY : stdout
    USE io_var,        ONLY : iospectral_sup, iospectral_cum
    USE input,         ONLY : nw_specfun, &
                              bnd_cum, nstemp, eliashberg
    USE global_var,    ONLY : ibndmin, ibndmax, gtemp
    !
    IMPLICIT NONE
    !
    CHARACTER(LEN = 20) :: tp
    ...
    !
    USE kinds,         ONLY : DP
    USE ep_constants,  ONLY : zero, one, two, ci, ryd2ev, ryd2mev
    USE ep_constants,  ONLY : pi
    USE input,         ONLY : degaussw, nw_specfun
    !
    IMPLICIT NONE
    !
