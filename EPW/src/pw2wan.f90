  ...
  CONTAINS
    ...
    ENDIF
    IF (scdm_sigma <= zero) &
      CALL errore('pw2wan90epw', 'Sigma in the SCDM method must be positive.', 1)
    !
    WRITE(stdout, *)
...
!    CALL phases_a_m()
  ...
  !      CLOSE(iummn)
    ...
    !!
    USE kinds,            ONLY : DP
    USE ep_constants,     ONLY : czero, cone, eps6
    USE wvfct,            ONLY : npwx
    USE wann_common,      ONLY : gf, n_proj, spin_qaxis, spin_eig, gf_spinor
    !
    IMPLICIT NONE
    !
    ...
    DO ik = 1, nks
      ...
      DO ibnd = 1, nbndep
        ...
        DO jbnd = 1, nbndep
          lbnd = ibndkept(jbnd)
          IF (include_nonlocal_) THEN
            dmec(:, jbnd, ibnd, ik) = dipole_aux(:, lbnd, hbnd)
          ELSE
            pmec(:, jbnd, ibnd, ik) = dipole_aux(:, lbnd, hbnd)
          ENDIF
        ENDDO
      ENDDO
      !
    ...
    USE fft_interfaces,  ONLY : invfft
    USE noncollin_module,ONLY : noncolin, npol
    USE ep_constants,    ONLY : czero, zero, twopi, ci
    USE mp_pools,        ONLY : my_pool_id
    USE kfold,           ONLY : ktokpmq
    USE io,              ONLY : readwfc
    USE mp_world,        ONLY : world_comm
    ...
    CONTAINS
      ...
      CHARACTER(LEN = 60) :: wancube_r
      !! ZD: Name of real part of Wannier function cube files
      CHARACTER(LEN = 60) :: wancube_i
      !! ZD: Name of imaginary part of Wannier function cube files
...
203   FORMAT(a, '_', i5.5, 'r.cube')! ZD: write real and imag part of Wanfunc
204   FORMAT(a, '_', i5.5, 'i.cube')! ZD: write real and imag part of Wanfunc
      !
      ...
      WRITE(iun_plot, '(9I10)') ngx, ngy, ngz, istart(1:3), iend(1:3)
      !
      ...
      CLOSE(iun_plot)
      !
      ! ZD: Writing only real part
      ! First two lines are comments
      IF(exciton) THEN
        OPEN(UNIT = iun_plot, FILE=TRIM(wancube_r), FORM='formatted', STATUS='unknown')
        ...
        DO nxx = 1, ilength(1)
          DO nyy = 1, ilength(2)
            DO nzz = 1, ilength(3)
              WRITE(iun_plot, '(E13.5)', ADVANCE = 'no') REAL(REAL(wann_func(nxx,nyy,nzz,1)))
              IF ((MOD(nzz, 6) == 0) .OR. (nzz == ilength(3))) WRITE(iun_plot, '(a)') ''
            ENDDO
          ENDDO
        ENDDO
        ...
        CLOSE(iun_plot)
        !
        ! ZD: Writing only imaginary part 
        ! First two lines are comments
        OPEN(UNIT = iun_plot, FILE=TRIM(wancube_i), FORM='formatted', STATUS='unknown')
        ...
        DO nxx = 1, ilength(1)
          DO nyy = 1, ilength(2)
            DO nzz = 1, ilength(3)
              WRITE(iun_plot, '(E13.5)', ADVANCE = 'no') REAL(AIMAG(wann_func(nxx,nyy,nzz,1)))
              IF ((MOD(nzz, 6) == 0) .OR. (nzz == ilength(3))) WRITE(iun_plot, '(a)') ''
            ENDDO
          ENDDO
        ENDDO
