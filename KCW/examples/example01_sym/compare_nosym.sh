...
check=1
echo 
echo " Testing the results against the same calculation without symmetry (example01)..." 
echo "  tolerances: "
...
a_ref=`grep ! ../example01/reference/Si.scf.out | awk '{print $5}'`
...
a_ref=`grep high ../example01/reference/Si.scf.out | awk '{print $7}'`
err=`echo $a $a_ref | awk '{printf "%20.15f \n", sqrt(($1-$2)*($1-$2))}'`
...
a_ref=`grep high ../example01/reference/Si.scf.out | awk '{print $8}'`
err=`echo $a $a_ref | awk '{printf "%20.15f \n", sqrt(($1-$2)*($1-$2))}'`
if (( $(echo "$err > $tol_eig" |bc -l) )); then
...
a_ref=`grep -A 4 "Final State" ../example01/reference/wann_occ/Si.wout | head -$i | tail -1 | awk '{print $11}'`
...
a_ref=`grep -A 4 "Final State" ../example01/reference/wann_emp/Si_emp.wout | head -$i | tail -1 | awk '{print $11}'`
