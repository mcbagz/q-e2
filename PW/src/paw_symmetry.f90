...
MODULE paw_symmetry
    !
    !! Routines for becsum symmetrization.
    !
    USE kinds,          ONLY : DP
...
SUBROUTINE PAW_symmetrize_ddd( ddd )
    ...
    !
    !
    REAL(DP), INTENT(INOUT) :: ddd(nhm*(nhm+1)/2,nat,nspin)
    !! cross band occupations
    !
    ! ... local variables
    !
...
SUBROUTINE PAW_desymmetrize( dbecsum )
    ...
    !
    !
    COMPLEX(DP), INTENT(INOUT) :: dbecsum(nhm*(nhm+1)/2,nat,nspin_mag,3)
    !! cross band occupations
    !
    ! ... local variables
    !
