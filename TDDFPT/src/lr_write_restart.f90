...
SUBROUTINE lr_write_restart()
  ...
  USE lr_variables,         ONLY : beta_store, gamma_store, zeta_store, norm0, &
                                   ...
                                   magnons, evc1_rgt, evc1_lft, evc1_rgt_old, &
                                   evc1_lft_old, n_op, alpha_magnons_store, &
                                   gamma_magnons_store
  ...
  !
  IF (eels .or. magnons) tmp_dir = tmp_dir_lr
  !
#if defined(__MPI)
  IF (ionode) THEN
#endif
  ...
  !-----------------------------------
  !
  ! write alpha_1 and zeta_1 for magnons
  !
  !
  IF (magnons) THEN
     !     
     WRITE(158,*) alpha_magnons_store(pol_index,1)
     !
     DO i = 1, n_op
        WRITE(158,*) zeta_store (pol_index, i, 1)
     ENDDO
     !
     DO i=1,LR_iteration-1
        !
        WRITE(158,*) alpha_magnons_store(pol_index,i+1)
        WRITE(158,*) beta_store(pol_index,i+1)
        WRITE(158,*) gamma_magnons_store(pol_index,i+1)
        !
        ! This is absolutely necessary for cross platform compatibility
        !
        DO j=1,n_op
           WRITE(158,*) zeta_store (pol_index,j,i+1)
        ENDDO
        !
     ENDDO
     !
  ...
  ENDIF
    ...
    IF (charge_response == 1 .AND. .NOT.eels .AND. .NOT. magnons) THEN
       ...
       !
    ENDIF
    IF (sum_rule == -2 .AND. .NOT.eels .AND. .NOT. magnons) THEN
       !
