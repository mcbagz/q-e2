...
!
!----------------------------------------------------------------------------
SUBROUTINE lr_dvpsi_magnons (ik, ip, dvpsi)
  !--------------------------------------------------------------------------
  !
  ...
  COMPLEX(DP), INTENT(OUT) :: dvpsi(npwx*npol,nbnd_occx,2)
  ...
  INTEGER :: ikk, &  ! index of the point k
             ...
             imk, &  ! index of the point -k
             imkq    ! index of the point -k-q
  ...
  !
  CALL start_clock ('lr_dvpsi_magnons') 
  !
  ...
  DO ibnd = 1, nbnd_occ(ikk), incr
     ...
     IF ( dffts%has_task_groups ) THEN
        ...
        !
        ! back-FFT to G-space
        CALL cft_wave_tg(ik, dvpsi(:,:,1), tg_psic, -1, v_siz, ibnd, nbnd_occ(ikk))
        !
...
!$acc end data
  ...
  ! Apply -P_c^+
  !
  CALL orthogonalize(dvpsi(:,:,1), evq, ikk, ikq, dpsi, npwq, .false.) 
  !
  ! Anti-Resonant Batch
  !
  ...
  DO ibnd = 1, nbnd_occ(imk), incr
     ...
     IF ( dffts%has_task_groups ) THEN
        ...
        !
        ! back-FFT to G-space
        CALL cft_wave_tg(ik, dvpsi(:,:,2), tg_psic, -1, v_siz, ibnd, nbnd_occ(imk))
        !
...
!$acc end data
  ...
  ENDIF
  !
  CALL stop_clock ('lr_dvpsi_magnons')
  !
  RETURN
  !
END SUBROUTINE lr_dvpsi_magnons
