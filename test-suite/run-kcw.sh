...
then
  ...
  #echo "${ESPRESSO_BUILD}/bin/wannier90.x -pp $2 "
  ${ESPRESSO_BUILD}/bin/wannier90.x -pp $2 
  cp *nnkp $3
  if [[ -e Si.werr ]]
  then
    cat Si.werr
...
then
  ...
  #echo "${PARA_PREFIX} ${ESPRESSO_BUILD}/bin/pw2wannier90.x < $2 > $3 2>$4 "
  ${PARA_PREFIX} ${ESPRESSO_BUILD}/bin/pw2wannier90.x < $2 > $3 2>$4
  cp *eig $3
  if [[ -e CRASH ]]
  then
    cat $3
...
then
  ...
  #echo "${ESPRESSO_BUILD}/bin/wannier90.x $2 "
  ${ESPRESSO_BUILD}/bin/wannier90.x  $2 
  cp *.wout $3
  if [[ -e Si.werr ]]
  then
    cat Si.werr
...
then
  ...
  #echo "${PARA_PREFIX} ${ESPRESSO_BUILD}/bin/kcw.x ${PARA_SUFFIX}< $2 > $3 2> $4"
